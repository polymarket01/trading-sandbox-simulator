...
class ContractLadderAdapter:
    ...
    async def _identity(self, session, symbol, uid, *, placing=False):
        ...
        if cached is not None and monotonic() - cached[0] < 1.0:
            _at, market, user, enabled = cached
            if placing and (not enabled or not user.is_active):
                raise ContractValidationError("internal maker identity is disabled")
            return market, user
        market = await self.service.get_market(session, str(symbol).upper())
        user = await session.get(User, int(uid))
        if user is None or not await self.service.is_contract_ladder(session, int(uid), int(market.id)):
            raise ContractValidationError("CONTRACT_LADDER server binding required")
    ...
    async def place(self, symbol, maker_uid, *, client_order_id, side, price, quantity):
        ...
        if task is None:
            ...
            task = asyncio.create_task(self._place(key, side, price, quantity), name=f"ladder-order-{key[0]}")
    ...
    async def _place(self, key, side, price, quantity):
        ...
        async with self.session_factory() as session:
            ...
            async with self.runtime.market_financial_guard(symbol, lambda: self._resource_keys(market, uid)):
                ...
                if any(o.user_id == uid and o.side != side and ((side == "buy" and o.price <= payload.price) or (side == "sell" and o.price >= payload.price)) for o in book.orders.values()):
                    raise ContractValidationError("own opposite order conflict; cancel confirmation required")
                before = {oid: dict(snap) for oid, snap in self.service._fast_contract_orders.items() if int(snap.get("market_id") or 0) == int(market.id)}
