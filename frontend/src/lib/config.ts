...
export const config = {
  apiBaseUrl: import.meta.env.VITE_API_BASE_URL ?? `${sameOriginBase}/api/v1`,
  publicWsUrl: import.meta.env.VITE_PUBLIC_WS_URL ?? `${sameOriginBase.replace(httpProtocol, wsProtocol)}/ws/public`,
  privateWsUrl: import.meta.env.VITE_PRIVATE_WS_URL ?? `${sameOriginBase.replace(httpProtocol, wsProtocol)}/ws/private`,
