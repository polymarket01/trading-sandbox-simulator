...
import { ContractOrderForm } from "../components/ContractOrderForm";
...
export function ContractFastTradePage() {
  ...
  const [contractSetting, setContractSetting] = useState<ContractSetting>();
  const [contractPriceState, setContractPriceState] = useState<ContractPriceState>();
  const [contractFundingEvents, setContractFundingEvents] = useState<ContractFundingEvent[]>([]);
  ...
  const contractMarketItems = marketItems.filter((item) => item.product_type === "PERP");
    ...
    makerInstance === undefined
      ...
      : makerInstance.running
        ? makerInstance.status === "stale"
          ...
          : "运行中"
        : "无运行实例";
  const displaySymbol = symbol.endsWith("USDT-PERP") ? symbol.replace("USDT-PERP", "/USDT-PERP") : symbol.replace("USDT", "/USDT");
  ...
  const refreshAll = async () => {
    ...
    try {
      const marketList = await api.get<{ items: MarketDefinition[] }>("/markets");
      const nextMarket = marketList.items.find((item) => item.symbol === symbol);
      if (nextMarket && nextMarket.product_type !== "PERP") {
        navigate(`/spot/terminal/${symbol}`);
        return;
      }
      const [
        tickerResponse,
        ...
        api.get<ContractAccount>("/contracts/account", accountApiKey),
        api.get<{ items: ContractPosition[] }>(`/contracts/positions?symbol=${symbol}`, accountApiKey),
        api.get<ContractSetting>(`/contracts/settings/${symbol}`, accountApiKey),
        api.get<ContractPriceState>(`/contracts/prices/${symbol}?refresh_external=true`),
        api.get<{ items: ContractFundingEvent[] }>(`/contracts/funding/events?symbol=${symbol}&limit=20`, accountApiKey),
        api.get<{ items: OrderItem[] }>(`/contracts/orders/open?symbol=${symbol}`, accountApiKey),
      ...
      startTransition(() => {
        ...
        setContractAccount(accountResponse);
        setContractPositions(positionsResponse.items);
        setContractSetting(settingResponse);
        setContractPriceState(priceStateResponse);
        setContractFundingEvents(fundingEventsResponse.items);
  ...
  useEffect(() => {
    ...
    setContractSetting(undefined);
    setContractPriceState(undefined);
    setContractFundingEvents([]);
  ...
  const cancelOrder = async (orderId: string) => {
    try {
      await api.delete(`/contracts/orders/${orderId}`, accountApiKey);
      await refreshAll();
      pushToast("success", "合约订单已撤销");
    } catch (error) {
  ...
  return (
    <AppShell>
      <ToastViewport />
      <section className="panel mb-3 overflow-hidden rounded-2xl px-4 py-3 lg:h-[144px] xl:h-[124px]">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex min-w-0 flex-wrap items-start gap-3">
            <div className="h-[82px] min-w-[210px]">
              <div className="flex items-center gap-2">
                <h1 className="font-display text-2xl text-white">{displaySymbol}</h1>
                <span className="rounded-lg bg-cyan-400/12 px-2 py-1 text-xs text-cyan-100">PERP 快速交易</span>
              </div>
              <select
                value={symbol}
                onChange={(event) => navigate(`/contracts/terminal/${event.target.value}`)}
                className="mt-2 h-9 w-full rounded-lg border border-white/10 bg-slate-950/55 px-3 text-sm text-slate-100 outline-none"
              >
                {contractMarketItems.map((item) => (
                  <option key={item.symbol} value={item.symbol}>
                    {item.symbol.replace("USDT-PERP", "/USDT-PERP")} · PERP · {item.market_type}
                  </option>
                ))}
              </select>
            </div>
            <Metric label="最新价" value={fmt(liveTicker?.last_price, priceDigits)} large tone={Number(liveTicker?.change_24h_pct ?? 0) < 0 ? "sell" : "buy"} />
            <Metric label="标记价" value={fmt(contractPriceState?.mark_price, priceDigits)} />
            <Metric label="指数价" value={fmt(contractPriceState?.index_price, priceDigits)} />
            <Metric label="资金费率" value={fmtPct(contractPriceState?.funding_rate, 4)} tone={Number(contractPriceState?.funding_rate ?? 0) < 0 ? "sell" : "buy"} />
            <Metric label="Spread" value={fmtPct(liveTicker?.spread_pct, 4)} />
            <Metric label="可用保证金" value={`${fmt(contractAccount?.available_margin, 2)} ${contractAccount?.margin_asset ?? market?.margin_asset ?? "USDT"}`} />
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => navigate(`/contracts/trade/${symbol}`)} className="rounded-lg bg-white/5 px-3 py-1.5 text-xs text-slate-300 hover:bg-white/10">
              交易台
            </button>
          </div>
        </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_300px_300px] 2xl:grid-cols-[minmax(620px,1fr)_340px_360px]">
        ...
        <aside className="min-h-0 space-y-3">
          <ContractOrderForm
            ...
            onSettingChanged={setContractSetting}
            onSubmitted={refreshAll}
          />
          <QuickContractAccount
            account={contractAccount}
            ...
            quantityDigits={quantityDigits}
          />
          <QuickContractOpenOrders orders={openOrders} priceDigits={priceDigits} quantityDigits={quantityDigits} onCancel={cancelOrder} />
        </aside>
      </div>
    </AppShell>
  );
...
function Metric({ label, value, large = false, tone = "neutral" }: { label: string; value: string; large?: boolean; tone?: "buy" | "sell" | "warn" | "neutral" }) {
  ...
  return (
    <div className={`${large ? "h-[54px] min-w-[150px]" : "h-[48px] min-w-[110px]"} overflow-hidden`}>
  ...
  return (
    <section className="panel rounded-2xl p-3">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          <h3 className="font-display text-base">合约保证金账户</h3>
          <div className="mt-0.5 text-xs text-slate-500">独立于现货钱包</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <AccountMetric label="未实现 PnL" value={fmt(account?.unrealized_pnl, 4)} tone={Number(account?.unrealized_pnl ?? 0) < 0 ? "sell" : "buy"} />
        <AccountMetric label="当前杠杆" value={`${setting?.leverage ?? setting?.default_leverage ?? "-"}x`} />
        <AccountMetric label="下次资金费" value={priceState?.next_funding_time ? bjTime(priceState.next_funding_time) : "-"} />
      </div>
      <div className="mt-3 space-y-2">
        {activePositions.slice(0, 4).map((item) => (
          <div key={`${item.symbol}-${item.side}`} className="rounded-xl bg-white/5 px-3 py-2 text-xs">
            <div className="flex items-center justify-between gap-2">
              <span className={item.side === "long" ? "text-emerald-300" : "text-rose-300"}>{item.side === "long" ? "多仓" : "空仓"}</span>
              <span className="font-mono text-slate-200">{fmt(item.quantity, quantityDigits)}</span>
            </div>
            <div className="mt-2 grid grid-cols-3 gap-2 font-mono tabular-nums">
              <span className="truncate text-slate-500">入 {fmt(item.entry_price, priceDigits)}</span>
              <span className="truncate text-slate-500">标 {fmt(item.mark_price, priceDigits)}</span>
              <span className={`truncate text-right ${Number(item.unrealized_pnl ?? 0) < 0 ? "text-rose-300" : "text-emerald-300"}`}>
                {fmt(item.unrealized_pnl, 4)}
              </span>
            </div>
          </div>
        ))}
        {activePositions.length === 0 && <div className="rounded-xl bg-slate-950/30 px-3 py-5 text-center text-sm text-slate-500">当前无合约持仓</div>}
      </div>
      {fundingEvents.length > 0 && (
        <div className="mt-3 rounded-xl bg-white/5 px-3 py-2 text-xs text-slate-400">
          最近资金费：{fmt(fundingEvents[0].amount, 6)} {fundingEvents[0].position_side} · {bjTime(fundingEvents[0].funding_time)}
        </div>
      )}
    </section>
  );
...
function AccountMetric({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "buy" | "sell" | "neutral" }) {
  const toneClass = tone === "buy" ? "text-emerald-300" : tone === "sell" ? "text-rose-300" : "text-slate-100";
  return (
    <div className="rounded-xl bg-white/5 px-3 py-2">
      <div className="text-slate-500">{label}</div>
      <div className={`mt-1 font-mono tabular-nums ${toneClass}`} title={value}><span className="num-fixed num-money">{value}</span></div>
    </div>
  );
}

function QuickContractOpenOrders({
  orders,
  priceDigits,
  quantityDigits,
  onCancel,
  ...
  return (
    <section className="panel rounded-2xl p-3">
      ...
      <div className="max-h-[220px] space-y-2 overflow-auto pr-1">
        {orders.slice(0, 8).map((item) => {
          ...
          return (
            <div key={item.order_id} className="rounded-xl bg-white/5 px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                <span className={`text-sm font-medium ${sideColor(item.side)}`}>
                  {item.side === "buy" ? "买入" : "卖出"} · {action} · {item.type}
                </span>
        ...
        {orders.length === 0 && <div className="rounded-xl bg-slate-950/30 px-3 py-6 text-center text-sm text-slate-500">暂无合约委托</div>}
      </div>
    </section>
  );
}
